...
export default async function AdminDashboard() {
  ...
  const { data: rawEvents } = await supabase.from('analytics_events').select('*');
  const events = rawEvents || [];

  const analytics = {
    visitors: events.filter(e => e.event_type === 'page_view').length,
    whatsappClicks: events.filter(e => e.event_type === 'whatsapp_click').length,
    productViews: {} as Record<string, number>,
    dailyVisitors: {} as Record<string, number>,
    referrers: {} as Record<string, number>
  };

  events.forEach(e => {
    if (e.event_type === 'product_view' && e.product_id) {
      analytics.productViews[e.product_id] = (analytics.productViews[e.product_id] || 0) + 1;
    }
    if (e.event_type === 'page_view') {
      // Convert UTC to simple date string logic
      const dateObj = new Date(e.created_at);
      const dateStr = dateObj.toISOString().split('T')[0];
      analytics.dailyVisitors[dateStr] = (analytics.dailyVisitors[dateStr] || 0) + 1;

      if (e.source && e.source !== 'Internal') {
        analytics.referrers[e.source] = (analytics.referrers[e.source] || 0) + 1;
      }
    }
  });
